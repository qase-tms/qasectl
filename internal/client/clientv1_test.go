...
func TestClientV1_UsesConfiguredHost(t *testing.T) {
	tests := []struct {
		name string
		host string
		want string
	}{
		{name: "custom host", host: "api.qase.example.com", want: "https://api.qase.example.com/v1"},
		{name: "empty host falls back to default", host: "", want: "https://api.qase.io/v1"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := NewClientV1("token", tt.host)

			_, apiClient := c.getApiV1Client(context.Background())
